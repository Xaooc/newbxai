...
* Во всех режимах (кроме shadow) план подтверждения не запрашивается: после генерации ACTION шаги выполняются последовательно, а предупреждения о рисках формируются постфактум.
...
* Если Bitrix24 вернул ошибку, агент выполняет до двух автоматических повторов с экспоненциальной задержкой. После окончательной неудачи агент преобразует ошибку в понятное пояснение (что сделать, какие данные уточнить) и фиксирует диагностическое сообщение в логах. Монитор ошибок отслеживает bursts за последние 60 секунд и при превышении порога (5 ошибок подряд) отправляет алерт в административный чат и логирует событие на уровне ERROR.
## План и отчётность
...
* Логи безопасности содержат список рискованных действий, ошибок Bitrix24 и рекомендации, которые агент выдал пользователю. Кроме того, туда попадает агрегированная статистика по недостающим данным и рискованным изменениям (топ-5 полей за период).
