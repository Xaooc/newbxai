...
9. При успешной валидации выполняется вызов Bitrix24. Результат передаётся в модуль состояния (обновление `objects`, добавление записей в `done`) и логируется вместе с планом.
10. Если Bitrix24 вернул ошибку или валидация провалилась, оркестратор формирует человеко-понятное пояснение: что пошло не так, какие данные попросить у пользователя и можно ли повторить шаг. Все ошибки собираются и возвращаются вместе с итоговым ответом.
...
* В системном промпте фиксируются ключевые инструкции: сформируй план действий и выполни его сразу, правило «для поиска сотрудников используем только `user.get`», «`crm.contact.*` — для клиентов и внешних контактов», напоминание об обязательных полях методов и требованиях к описанию ошибок без ссылок на внутренние ID.
...
* Шаги исполняются строго последовательно. При каждом вызове Bitrix24 оркестратор логирует метод, параметры (без чувствительных данных) и результат/ошибку.
* Для рискованных изменений (`OPPORTUNITY`, `ASSIGNED_BY_ID`, стадия сделки, дедлайны, подписки на события) в ответ пользователю добавляется предупреждение о характере действия и рекомендации перепроверить данные.
* Если ошибка возникла на шаге, оставшиеся шаги не прерываются автоматически: оркестратор продолжает план, но в ответе явно указывает, какие шаги завершились успешно, а какие — с ошибкой, и что нужно сделать (например, указать обязательные поля или проверить права).
