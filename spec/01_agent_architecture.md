...
* Дополнительные проверки убеждаются, что чтение справочников (`crm.status.list`, `crm.deal.category.*`, `sonet.group.*`) корректно обновляет историю `done` и сохраняет агрегированную статистику в состоянии.
